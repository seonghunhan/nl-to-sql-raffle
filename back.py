...
st.title(":tada: 엑셀 당첨자 추첨기")
# 엑셀 업로드
uploaded_file = st.file_uploader("엑셀 파일을 업로드하세요", type=["xlsx", "xls"])
if uploaded_file is not None:
    df = pd.read_excel(uploaded_file)
    st.subheader(":memo: 업로드된 데이터 미리보기")
    st.dataframe(df.head())
    # 자연어 조건 입력
    st.subheader(":mag: 조건을 자연어로 입력하세요")
    nl_query = st.text_input("예: '나이가 30 이상인 사람만', '여자만'")
    filtered_df = df.copy()
    sql_query = None
    if nl_query:
        sql_query = convert_to_sql(nl_query, df, table_name="df")
        st.markdown(f":point_right: 변환된 SQL 쿼리:\n```sql\n{sql_query}\n```")
        try:
            filtered_df = sqldf(sql_query, {"df": df})
            st.subheader(":bar_chart: 조건이 적용된 데이터")
            # :white_check_mark: 조회된 총 인원 수 표시
            st.write(f":white_check_mark: 총 {len(filtered_df)}명이 조회되었습니다.")
            # :white_check_mark: 데이터 표시 옵션 선택
    ...
    if not filtered_df.empty:
        ...
        if st.button("추첨하기"):
            ...
            st.subheader(":trophy: 당첨자 명단")
            st.dataframe(winners)
            csv = winners.to_csv(index=False).encode("utf-8-sig")
            st.download_button(
                label=":inbox_tray: 당첨자 명단 다운로드 (CSV)",
                data=csv,
                file_name="winners.csv",
                mime="text/csv",
            )
    else:
        st.warning(":warning: 조건에 맞는 데이터가 없습니다. 다른 조건을 입력해보세요.")